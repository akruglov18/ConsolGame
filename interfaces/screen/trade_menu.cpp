...
void TradeMenu::bind(const GraphicInventory& _gr_inventory_player,
                     const sf::Text& _gr_money_player) {
    gr_inventory_player = _gr_inventory_player;
...
void TradeMenu::update_graphic_inventories(const std::vector<std::shared_ptr<Slot>>& items_array_player,
    ...
    for (std::size_t i = 0; i < items_array_player.size(); i++) {
        ...
        if (gr_inventory_player[i]->slot->get_item() != nullptr) {
            ...
            gr_inventory_player[i]->gr_amount.setString(std::to_string(gr_inventory_player[i]->slot->get_amount()));
