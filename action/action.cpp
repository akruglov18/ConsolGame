...
void Action::move_left(Creature* creature, float time, const Field& game_field) {
    ...
    for (int i = 0; i < creature->get_armor().size(); ++i) {
        if (creature->get_armor()[i] != nullptr) {
            creature->get_armor()[i]->get_sprite().setPosition(sf::Vector2f(pos.x, pos.y - 32));
...
void Action::stop(Creature* creature) {
    ...
    switch (creature->_direction) {
        case(Dirs::LEFT):
            ...
            for (int i = 0; i < creature->get_armor().size(); ++i) {
                if (creature->get_armor()[i] != nullptr) {
                    creature->get_armor()[i]->get_sprite().setTextureRect(sf::IntRect(0, 64, 64, 64));
                }
            }
        ...
        case(Dirs::RIGHT):
            ...
            for (int i = 0; i < creature->get_armor().size(); ++i) {
                if (creature->get_armor()[i] != nullptr) {
                    creature->get_armor()[i]->get_sprite().setTextureRect(sf::IntRect(0, 192, 64, 64));
                }
            }
        ...
        case(Dirs::UP):
            ...
            for (int i = 0; i < creature->get_armor().size(); ++i) {
                if (creature->get_armor()[i] != nullptr) {
                    creature->get_armor()[i]->get_sprite().setTextureRect(sf::IntRect(0, 0, 64, 64));
                }
            }
        ...
        case(Dirs::DOWN):
            ...
            for (int i = 0; i < creature->get_armor().size(); ++i) {
                if (creature->get_armor()[i] != nullptr) {
                    creature->get_armor()[i]->get_sprite().setTextureRect(sf::IntRect(0, 128, 64, 64));
                }
            }
